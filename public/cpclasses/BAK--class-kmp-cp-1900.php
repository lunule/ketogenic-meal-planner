...
/**
 * The calorie possibilities class if the calorie target specified by the user is 1900.
 *
	...
	public function return_meals_1_no() {
		...
		ob_start();
			...
			for ($i = 0; $i < $this->number_of_cards; $i++ ) :
				...
				/**
				 * If there's no result for an object, let's just ignore the meal
				 * category (breakfast/dinner/etc.), and focus on the calorie range
				 * value.
				 *
				 * For 'dessert' or 'snack', let's add an error message to the
				 * errorsObj object.
				 */
				...
				if ( count($dinners_Arr) > 0 ) {
					...
					$din_randkeys_Arr 		= array_rand( $dinners_Arr, 2 );
					$dinner_pt1 			= $dinners_Arr[$din_randkeys_Arr[0]];
					$dinner_pt2 			= $dinners_Arr[$din_randkeys_Arr[1]];
		...
		ob_start();
			...
			for ($i = 0; $i < $this->number_of_cards; $i++ ) :
				...
				$breakfasts_Arr = array_filter($sub_results_Arr, function($value, $key) {
					return ( 'breakfast' == strtolower( $value->meal ) );
				}, ARRAY_FILTER_USE_BOTH);

				$dinners_Arr = array_filter($sub_results_Arr, function($value, $key) {
					return ( 'dinner' == strtolower( $value->meal ) );
				}, ARRAY_FILTER_USE_BOTH);
		...
		ob_start();
			...
			for ($i = 0; $i < $this->number_of_cards; $i++ ) :
				...
				if ( count($snacks_Arr) == 0 ) :
					...
					$errors_Arr['snack_error'] = 'We couldn\'t find a dessert with the selected criteria';
