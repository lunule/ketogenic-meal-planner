...
/**
 * The calorie possibilities class if the calorie target specified by the user is 1300.
 *
			...
			for ($i = 0; $i < $this->number_of_cards; $i++ ) :
				...
				if (
					...
					if (
						...
						$din_pt1_randkeys_Arr 	= array_rand( $dinners_pt1_Arr );
						$din_pt2_randkeys_Arr 	= array_rand( $dinners_pt2_Arr );

						$dinner_pt1 			= $dinners_pt1_Arr[$din_pt1_randkeys_Arr];
						$dinner_pt2 			= $dinners_pt2_Arr[$din_pt2_randkeys_Arr];
					...
					elseif (
						...
						// GET THE TWO RANDOM ELEMENTS THAT WILL NEED TO BE MERGED
						$din_randkeys_Arr 	= array_rand( $dinners_pt1_Arr, 2 );
						$dinner_pt1 		= $dinners_pt1_Arr[$din_randkeys_Arr[0]];
						$dinner_pt2 		= $dinners_pt1_Arr[$din_randkeys_Arr[1]];
					...
					elseif (
						...
						// GET THE TWO RANDOM ELEMENTS THAT WILL NEED TO BE MERGED
						$din_randkeys_Arr 	= array_rand( $dinners_pt2_Arr, 2 );
						$dinner_pt1 		= $dinners_pt2_Arr[$din_randkeys_Arr[0]];
						$dinner_pt2 		= $dinners_pt2_Arr[$din_randkeys_Arr[1]];
					...
					$dinner->id = $dinner_pt1->id . '|' . $dinner_pt2->id;
					$dinner->calorierange = $dinner_pt1->calorierange . '|' . $dinner_pt2->calorierange;
					$dinner->kmpid = $dinner_pt1->kmpid . '|' . $dinner_pt2->kmpid;
					$dinner->meal = $dinner_pt1->meal . '|' . $dinner_pt2->meal;
					$dinner->name = $dinner_pt1->name . '|' . $dinner_pt2->name;
					$dinner->ingredients = $dinner_pt1->ingredients . '|' . $dinner_pt2->ingredients;
					$dinner->calories = $dinner_pt1->calories  . '|' .  $dinner_pt2->calories;
					$dinner->fat = $dinner_pt1->fat  . '|' .  $dinner_pt2->fat;
					$dinner->carbs = $dinner_pt1->carbs  . '|' .  $dinner_pt2->carbs;
					$dinner->fiber = $dinner_pt1->fiber  . '|' .  $dinner_pt2->fiber;
					$dinner->netcarbs = $dinner_pt1->netcarbs  . '|' .  $dinner_pt2->netcarbs;
					$dinner->protein = $dinner_pt1->protein  . '|' .  $dinner_pt2->protein;
					$dinner->vegetarian = $dinner_pt1->vegetarian  . '|' .  $dinner_pt2->vegetarian;
					$dinner->vegan = $dinner_pt1->vegan  . '|' .  $dinner_pt2->vegan;
					$dinner->carnivore = $dinner_pt1->carnivore  . '|' .  $dinner_pt2->carnivore;
					$dinner->dairyfree = $dinner_pt1->dairyfree  . '|' .  $dinner_pt2->dairyfree;
					$dinner->eggfree = $dinner_pt1->eggfree  . '|' .  $dinner_pt2->eggfree;
					$dinner->beginnerfriendly = $dinner_pt1->beginnerfriendly  . '|' .  $dinner_pt2->beginnerfriendly;
					$dinner->advanced = $dinner_pt1->advanced  . '|' .  $dinner_pt2->advanced;
					$dinner->url = $dinner_pt1->url  . '|' .  $dinner_pt2->url;
					$dinner->posturl = $dinner_pt1->posturl  . '|' .  $dinner_pt2->posturl;
	...
	public function return_meals_3_dessert() {
		...
		ob_start();
			...
			$sub_results_Arr= array_filter($this->results, function($value, $key) {
				return (
					( 200 == $value->calorierange ) ||
					( 300 == $value->calorierange ) ||
					( 400 == $value->calorierange )
				);
			}, ARRAY_FILTER_USE_BOTH);
		...
		ob_start();
			...
			for ($i = 0; $i < $this->number_of_cards; $i++ ) :
				...
				if ( count($desserts_Arr) > 0 ) :
					...
					// GET THE TWO RANDOM ELEMENTS THAT WILL NEED TO BE MERGED
					$des_randkeys_Arr 	= array_rand( $desserts_Arr, 2 );
					$dessert_pt1 		= $desserts_Arr[$des_randkeys_Arr[0]];
					$dessert_pt2 		= $desserts_Arr[$des_randkeys_Arr[1]];

					// UPDATE MERGE VALUE
					$merge_Arr['dessert_merge'] = true;

					// CREATE NEW OBJECT
					$dessert = new stdClass();

					// SET UP OBJECT VALUES
					$dessert->id = $dessert_pt1->id . '|' . $dessert_pt2->id;
					$dessert->calorierange = $dessert_pt1->calorierange . '|' . $dessert_pt2->calorierange;
					$dessert->kmpid = $dessert_pt1->kmpid . '|' . $dessert_pt2->kmpid;
					$dessert->meal = $dessert_pt1->meal . '|' . $dessert_pt2->meal;
					$dessert->name = $dessert_pt1->name . '|' . $dessert_pt2->name;
					$dessert->ingredients = $dessert_pt1->ingredients . '|' . $dessert_pt2->ingredients;
					$dessert->calories = $dessert_pt1->calories  . '|' .  $dessert_pt2->calories;
					$dessert->fat = $dessert_pt1->fat  . '|' .  $dessert_pt2->fat;
					$dessert->carbs = $dessert_pt1->carbs  . '|' .  $dessert_pt2->carbs;
					$dessert->fiber = $dessert_pt1->fiber  . '|' .  $dessert_pt2->fiber;
					$dessert->netcarbs = $dessert_pt1->netcarbs  . '|' .  $dessert_pt2->netcarbs;
					$dessert->protein = $dessert_pt1->protein  . '|' .  $dessert_pt2->protein;
					$dessert->vegetarian = $dessert_pt1->vegetarian  . '|' .  $dessert_pt2->vegetarian;
					$dessert->vegan = $dessert_pt1->vegan  . '|' .  $dessert_pt2->vegan;
					$dessert->carnivore = $dessert_pt1->carnivore  . '|' .  $dessert_pt2->carnivore;
					$dessert->dairyfree = $dessert_pt1->dairyfree  . '|' .  $dessert_pt2->dairyfree;
					$dessert->eggfree = $dessert_pt1->eggfree  . '|' .  $dessert_pt2->eggfree;
					$dessert->beginnerfriendly = $dessert_pt1->beginnerfriendly  . '|' .  $dessert_pt2->beginnerfriendly;
					$dessert->advanced = $dessert_pt1->advanced  . '|' .  $dessert_pt2->advanced;
					$dessert->url = $dessert_pt1->url  . '|' .  $dessert_pt2->url;
					$dessert->posturl = $dessert_pt1->posturl  . '|' .  $dessert_pt2->posturl;
		...
		ob_start();
			...
			for ($i = 0; $i < $this->number_of_cards; $i++ ) :
				...
				if ( count($snacks_Arr) > 0 ) :
					...
					// GET THE TWO RANDOM ELEMENTS THAT WILL NEED TO BE MERGED
					$sna_randkeys_Arr 	= array_rand( $snacks_Arr, 2 );
					$snack_pt1 		= $snacks_Arr[$sna_randkeys_Arr[0]];
					$snack_pt2 		= $snacks_Arr[$sna_randkeys_Arr[1]];

					// UPDATE MERGE VALUE
					$merge_Arr['snack_merge'] = true;

					// CREATE NEW OBJECT
					$snack = new stdClass();

					// SET UP OBJECT VALUES
					$snack->id = $snack_pt1->id . '|' . $snack_pt2->id;
					$snack->calorierange = $snack_pt1->calorierange . '|' . $snack_pt2->calorierange;
					$snack->kmpid = $snack_pt1->kmpid . '|' . $snack_pt2->kmpid;
					$snack->meal = $snack_pt1->meal . '|' . $snack_pt2->meal;
					$snack->name = $snack_pt1->name . '|' . $snack_pt2->name;
					$snack->ingredients = $snack_pt1->ingredients . '|' . $snack_pt2->ingredients;
					$snack->calories = $snack_pt1->calories  . '|' .  $snack_pt2->calories;
					$snack->fat = $snack_pt1->fat  . '|' .  $snack_pt2->fat;
					$snack->carbs = $snack_pt1->carbs  . '|' .  $snack_pt2->carbs;
					$snack->fiber = $snack_pt1->fiber  . '|' .  $snack_pt2->fiber;
					$snack->netcarbs = $snack_pt1->netcarbs  . '|' .  $snack_pt2->netcarbs;
					$snack->protein = $snack_pt1->protein  . '|' .  $snack_pt2->protein;
					$snack->vegetarian = $snack_pt1->vegetarian  . '|' .  $snack_pt2->vegetarian;
					$snack->vegan = $snack_pt1->vegan  . '|' .  $snack_pt2->vegan;
					$snack->carnivore = $snack_pt1->carnivore  . '|' .  $snack_pt2->carnivore;
					$snack->dairyfree = $snack_pt1->dairyfree  . '|' .  $snack_pt2->dairyfree;
					$snack->eggfree = $snack_pt1->eggfree  . '|' .  $snack_pt2->eggfree;
					$snack->beginnerfriendly = $snack_pt1->beginnerfriendly  . '|' .  $snack_pt2->beginnerfriendly;
					$snack->advanced = $snack_pt1->advanced  . '|' .  $snack_pt2->advanced;
					$snack->url = $snack_pt1->url  . '|' .  $snack_pt2->url;
					$snack->posturl = $snack_pt1->posturl  . '|' .  $snack_pt2->posturl;
